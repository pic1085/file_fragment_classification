...
# 확장자 라벨(0~74) -> 카테고리 id(0~9)
LABEL2CATID = np.array([CAT2ID[s.split(":")[1]] for s in INDEX2NAME], dtype=np.int64)
...
class LightweightFFTDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.augment and np.random.random() > 0.5:
            ...
            shift = np.random.randint(-4, 4) # 크기 낮춤 -10~10 -> -4~4
            fragment = np.roll(fragment, shift)
        
        fragment = torch.from_numpy(fragment).long()
        orig_label = int(self.y_orig[idx])
        new_label = int(LABEL2CATID[orig_label])
...
def eval_model(model, loader, criterion, epoch, phase="Val"):
    ...
    for cls in range(NUM_CLASSES):
        ...
        if mask.sum() > 0:
            ...
            cls_acc = 0
        per_cls.append(cls_acc)

    print("Per-category accuracy:")
    for i, a in enumerate(per_cls):
        print(f" - {ID2CAT[i]:15s}: {a*100:6.2f}%")
    
    return avg_loss, avg_acc, macro_acc, per_cls
...
def main():
    ...
    with open(csv_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(
            ["epoch", "train_loss", "train_acc", "val_loss", "val_acc", "val_macc"] 
            + [f"val_{c}_acc" for c in CATEGORIES]
        )
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        val_loss, val_acc, val_macc, val_per_cls = eval_model(
        ...
        with open(csv_path, 'a', newline="") as f:
            ...
            writer.writerow(
                ...
                + [f"{a:.4f}" for a in val_per_cls]
            )
        
        print(f"[Summary] Train Acc: {train_acc*100:.2f}% | Val Acc: {val_acc*100:.2f}%")
        print("-" * 50)
        print(f"Per-class Val Acc: {[f'{a*100:.2f}%' for a in val_per_cls]}")
        # Best model 저장
    ...
    print("\n" + "=" * 50)
    print("FINAL TEST")
    test_loss, test_acc, test_macc, test_per_cls = eval_model(
    ...
    # 모델 저장
    save_path = f"{model_name}_cluster_RollDown_best.pth"
    torch.save({
