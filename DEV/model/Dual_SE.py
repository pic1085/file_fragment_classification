"""
- 모델: Lightweight 1D CNN (Depthwise Separable + SE Block, DSC-SE 스타일)
"""
...
class FFTSubsetDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        fragment = self.x[idx].long()   # (4096,)  -> embedding 입력용 long

        orig_label = int(self.y_orig[idx])       # 예: 16,21,23,39,45,...
        new_label = LABEL2INDEX[orig_label]      # 0~10으로 매핑
...
class DualSEBlock(nn.Module):
    def __init__(self, channels, reduction=16, seq_len=4096):
        super().__init__()

        # reduction이 너무 크면 0 되는 걸 방지
        hidden_c = max(1, channels // reduction)
        hidden_p = max(1, seq_len // reduction)
        ...
        # ----- Position SE -----
        # pos_pool은 지금 구조에서는 안 쓰이므로 제거해도 됨
        # self.pos_pool = nn.AdaptiveAvgPool1d(1)

        self.fc_pos1 = nn.Linear(seq_len, hidden_p)
        self.fc_pos2 = nn.Linear(hidden_p, seq_len)
    ...
    def forward(self, x):
        ...
        # 채널 평균 → 위치별 스케일
        p = x.mean(dim=1)                        # (B, L)
        p = F.relu(self.fc_pos1(p))
        p = torch.sigmoid(self.fc_pos2(p))
        p = p.view(B, 1, L)                      # (B, 1, L)

        return x * y * p


# ==========================
# Inception + DSC Block (기존 그대로)
# ==========================
...
class DSCSEClassifier(nn.Module):
    ...
    - Conv1d
    - InceptionDSCBlock x 3 + DualSEBlock
    - Global AvgPool
    - 1x1 Conv -> num_classes
    """
    def __init__(self, num_classes=NUM_CLASSES, emb_dim=32):
        super().__init__()
        self.emb = nn.Embedding(256, emb_dim)   # byte embedding
        ...
        self.se1    = DualSEBlock(64, reduction=16, seq_len=FRAGMENT_SIZE)

        self.block2 = InceptionDSCBlock(64, 64)
        self.se2    = DualSEBlock(64, reduction=16, seq_len=FRAGMENT_SIZE)

        self.block3 = InceptionDSCBlock(64, 128)
        self.se3    = DualSEBlock(128, reduction=16, seq_len=FRAGMENT_SIZE)
...
def eval_model(model, loader, criterion, epoch: int, phase: str = "Val"):
    ...
    for x, y in tqdm(loader, desc=f"{phase} {epoch:02d}", leave=False):
        ...
        # 저장(계산은 GPU/CPU 혼합되면 안되니까 CPU로 이동)
        preds = logits.argmax(dim=1)
...
def save_model(model, path="dscse_fft11_best.pth"):
    # DataParallel 사용 시 model.module에 실제 가중치가 들어 있음
...
def main():
    ...
    # GPU 두 개 이상이면 DataParallel로 자동 분산
    ...
    # Label Smoothing 적용 (0.1 정도가 많이 쓰이는 값)
    criterion = nn.CrossEntropyLoss(label_smoothing=0.1)

    # weight_decay로 약간의 L2 정규화도 같이 주면 일반화에 도움됨
    ...
    save_model(model, "Dual-SE_best.pth")
    print("Saved model to Dual-SE_best.pth")
